...
f_min=+inf;
init_guess=log([A_SE;M_SE]);
for i=1:20
    [hyper_parameter_possible,f]=fminunc(@(hyper_parameter)Kernel_SE(X_r,y,sigma2_N,hyper_parameter),init_guess,options);
    if(f<f_min)
        f_min=f;
...
for i=1:length(X_star)
    K_star=A_SE*exp(-M_SE*(repmat(X_star(i),n,1)-X).^2);
    y_m(i)=K_star.'*K_inv*y;
    y_sigma(i)=sqrt(A_SE+sigma2_N-K_star.'*K_inv*K_star);
end
%% plot
